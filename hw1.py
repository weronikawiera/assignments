...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    pogrupowane = confirmed_cases.groupby("Country/Region").sum()
    result = pogrupowane.sort_values(by=f"{month}/{day}/20").tail(5)
    lista = result.sort_values(by=f"{month}/{day}/20", ascending=False)
